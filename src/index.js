...
window.changeUnits = () => {
  ...
  for (var i = 0, len = temp.length; i < len; i++){
    ...
    if (tempValue.textContent.indexOf('℃') !== -1) {
      let mainTemp = parseInt(tempValue.textContent.replace('℃', ''))
      let sol = Math.round(((mainTemp / 5)* 9) + 32)
      tempValue.textContent = `${sol}°F`
      unitsBtn.innerHTML = '<span class="text-secondary">℃</span> / <span>°F</span>'

    }else {
      let mainTemp = parseInt(tempValue.textContent.replace('°F', ''))
      let sol = Math.round(((mainTemp-32)/9)*5)
